	...
	if (DOES_ENTITY_EXIST(wagon_spawned_vehicle))
	{
		...
		if (_UIPROMPT_IS_VALID(wagon_prompt))
		{
			if (_UIPROMPT_HAS_HOLD_MODE_COMPLETED(wagon_prompt))
			{
				...
				int index = -1;
				int getArrayLength = sizeof(stowed_animals) / sizeof(stowed_animals[0]);
				for (int i = 0; i < getArrayLength; i++) {
					if (stowed_animals[i] == 0) {
						index = i;
						break;
					}
				}
				if (index != -1) {
					stowed_animals[index] = holding_entity;
				}
		...
		if (_UIPROMPT_IS_VALID(wagon_removal_prompt))
		{
			if (_UIPROMPT_HAS_HOLD_MODE_COMPLETED(wagon_removal_prompt))
			{
				int pickup_animal;
				ANIMPOSTFX_PLAY("CamTransition01");
				...
				if (index != -1) {
					pickup_animal = stowed_animals[index];
					stowed_animals[index] = 0;
				}

				Ped ped = GET_PLAYER_PED(PLAYER_ID());
				TASK_PICKUP_CARRIABLE_ENTITY(ped, pickup_animal);
				

				_UIPROMPT_DELETE(wagon_removal_prompt);
				wagon_removal_prompt = 0;
				Log::Write(Log::Type::Normal, "_PROMPT_DELETE");
			}
		}
...
void wagon_pickup_action(Entity entity_holding)
{
	...
	if (index != -1 && !DOES_ENTITY_EXIST(entity_holding) && !_UIPROMPT_IS_VALID(wagon_removal_prompt)  && !IS_PED_IN_VEHICLE(PLAYER_PED_ID(), wagon_spawned_vehicle, true) && DOES_ENTITY_EXIST(wagon_spawned_vehicle)) {
		wagon_removal_prompt = _UIPROMPT_REGISTER_BEGIN();
		_UIPROMPT_SET_CONTROL_ACTION(wagon_removal_prompt, INPUT_OPEN_SATCHEL_MENU);
		_UIPROMPT_SET_TEXT(wagon_removal_prompt, _CREATE_VAR_STRING(10, "LITERAL_STRING", "Pickup"));
		_UIPROMPT_SET_PRIORITY(wagon_removal_prompt, 1);
		_UIPROMPT_SET_TRANSPORT_MODE(wagon_removal_prompt, 0);
		_UIPROMPT_SET_ATTRIBUTE(wagon_removal_prompt, 18, 1);
		_UIPROMPT_SET_STANDARDIZED_HOLD_MODE(wagon_removal_prompt, 1704213876);
		_UIPROMPT_REGISTER_END(wagon_removal_prompt);

		_UIPROMPT_SET_ENABLED(wagon_removal_prompt, true);
		_UIPROMPT_SET_VISIBLE(wagon_removal_prompt, true);

		_UIPROMPT_SET_GROUP(wagon_removal_prompt, _UIPROMPT_GET_GROUP_ID_FOR_TARGET_ENTITY(wagon_spawned_vehicle), 0);
		Log::Write(Log::Type::Normal, "Created pickup prompt");
	}
	else if (IS_PED_IN_VEHICLE(PLAYER_PED_ID(), wagon_spawned_vehicle, true) || DOES_ENTITY_EXIST(entity_holding)) {
		_UIPROMPT_DELETE(wagon_removal_prompt);
		wagon_removal_prompt = 0;
	}
